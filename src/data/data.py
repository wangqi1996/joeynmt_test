...
from src.data.vocabulary import Vocabulary, build_vocab
from src.util.constants import PAD_TOKEN, UNK_TOKEN, BOS_TOKEN, EOS_TOKEN
...
def load_data(data_cfg: dict) -> (Dataset, Dataset, Optional[Dataset],
    ...
    trg_vocab = build_vocab(field="trg", max_size=trg_max_size, min_freq=trg_min_freq,
    ...
    if test_path is not None:
        ...
        if os.path.isfile(test_path + "." + trg_lang):
            ...
            # no atrget is given -> create dataset from src only
            test_data = MonoDataset(path=test_path, ext=("." + src_lang), field=src_field)
...
def token_batch_size_fn(new, count, sofar):
    ...
    if hasattr(new, 'trg'):  # for monolingual data sets ("translate" mode)
        ...
        trg_elements = count * max_tgt_in_batch
    else:
        trg_elements = 0
...
class MonoDataset(Dataset):
    ...
    def __init__(self, path: str, ext: str, field: Field, **kwargs) -> None:
        ...
        for src_line in src_file:
            ...
            if src_line != '':
                examples.append(
                    data.Example.fromlist([src_line], fields)
                )
