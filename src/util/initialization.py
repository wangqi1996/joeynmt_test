...
def orthogonal_rnn_init_(cell: nn.RNNBase, gain: float = 1.):
    """
    Orthogonal(正交) initialization of recurrent weights
    RNN parameters contain 3 or 4 matrices in one parameter, so we slice(切开) it.
    """
...
def initialize_model(model: nn.Module, cfg: dict, src_padding_idx: int,
    ...
    All initializer configuration part of the model section of the configuration file

    the main initializer is set using the 'initializer' key.
    Possible values are 'xavier', 'uniform', 'normal' or 'zeros'
    ('xavier' is the fefault)

    when an initializer is set to 'uniform', the 'init_weight' sets the range for the values
    ...
    def _parser_init(s, scale, _gain):
        scale = float(scale)
    ...
    init_fn_ = _parser_init(init, init_weight, gain)
    embed_init_fn_ = _parser_init(embed_init, embed_init_weight, embed_gain)
    bias_init_fn_ = _parser_init(bias_init, bias_init_weight, gain)
    ...
    # encoder rnn orthogonal initializer & LSTM forget gate
